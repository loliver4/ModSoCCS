...
file_handler = logging.FileHandler(
    "/projects/loliver/ModSoCCS/data/logs/parse_EA_task_AFNI_ZHP.log"
)
...
def convert_1D_format(sub_inpath, subid, sub_outpath):
    ...
    input_dir    a fullpath to subject directory that contain the EA task tsv
                (i.e /archive/data/SPN20/data/nii/SPN01_ZHP_0002_01)

    out_dir      a fullpath to subject output directory
                (i.e /projects/loliver/ModSoCCS/data/processed/sub-ZHP0002)

    """

    subj = "sub-" + subid
    tsv_files = sorted(glob(sub_inpath + "/" + "*EMP_*.tsv")) 
    print(tsv_files)
    if len(tsv_files) != 2:
        logger.critical(
    ...
    for f in tsv_files:
        ...
        event_df.loc[:, "onset"] = event_df["onset"].apply(lambda x: x - 9.2) # drops the first 4 TRs (3.2 s) plus 6 s after the trigger but before data acquisition (dummy scans) - this 6 s delay only occurred for ZHP scans 
...
def main():
    ...
    parser.add_argument(
        "sub_id", type=str, help='string of subject ID (i.e "ZHP0002")'
    )
