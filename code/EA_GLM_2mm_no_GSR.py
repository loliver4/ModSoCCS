...
file_handler = logging.FileHandler(
    "/projects/loliver/ModSoCCS/data/logs/EA_GLM_2mm_no_GSR.log"
)
...
def run_3Ddeconvolve(sub_dir, out_dir, sub_id):
    ...
    deconvolve.inputs.force_TR = 0.8
    deconvolve.inputs.polort_A = "A"
    deconvolve.inputs.num_stimts = 4
    deconvolve.inputs.local_times = True
    #deconvolve.inputs.stim_times_subtract = -0.4 # no slice timing correction, so this is probably unnecessary 
    ...
    deconvolve.inputs.goforit = 2 # changed to 5 for participant with no circles button presses, but was 2 (good to alert you when there are multiple issues)  
    workflow = pe.Workflow(name=sub_id)
    workflow.base_dir = "/projects/loliver/ModSoCCS/data/tmp/"
