...
file_handler = logging.FileHandler(
    "/projects/loliver/ModSoCCS/data/logs/parse_EA_task_AFNI.log"
)
...
def convert_1D_format(sub_inpath, subid, sub_outpath):
    ...
    input_dir    a full path to subject directory that contains the EA task tsvs
                (i.e /archive/data/SPN20/data/nii/SPN01_CMH_0012_01)

    out_dir      a full path to subject output directory
                (i.e /projects/loliver/ModSoCCS/data/processed/sub-CMH0012)

    """

    subj = "sub-" + subid
    tsv_files = sorted(glob(sub_inpath + "/" + "*EMP_*.tsv"))  # was *EAtask previously - don't just run everyone with this as does not guarantee order atm
    print(tsv_files)
    if len(tsv_files) != 2:
        logger.critical(
    ...
    for f in tsv_files:
        ...
        event_df.loc[:, "onset"] = event_df["onset"].apply(lambda x: x - 3.2) # drops the first 4 TRs (3.2 s)
