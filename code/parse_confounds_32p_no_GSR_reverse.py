...
def format_confounds_tsv(sub_inpath, subid, sub_outpath):
    ...
    tsv_files = sorted(
        glob(sub_inpath + "/" + "*task-emp*confounds_timeseries.tsv"), reverse=True)  # only reversed for CMH0003 atm
    print(tsv_files)
 ...
 #          combined_confounds_df[2], # only 2 EA runs
    ...
    output = os.path.join(sub_outpath, subj + "_ea_confounds_no_GSR_glm_rev.1D")
    print(output)
